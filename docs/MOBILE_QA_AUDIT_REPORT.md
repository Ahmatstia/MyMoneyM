...
- **2026-09-24 (Form UX & Input Autonomy Audit):** Conducted exhaustive codebase audit across all form screens. Removed all auto-assigned categories and system overrides (in Recurring Transactions, Debt, Split Bill, Savings, and Wallets), restoring 100% user autonomy over category and input selection without breaking existing features. Verified clean compilation with `tsc --noEmit` (exit 0).
